<x-admin-layout>
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2">Создание подкатегории</h1>
            <p class="text-gray-400">Добавьте новую подкатегорию (например, VPS, VDS)</p>
        </div>
    ...
    <div class="bg-[#0f0f13] border border-white/5 rounded-2xl p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.infrastructure.subcategories.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="infrastructure_category_id" id="infrastructure_category_id" required 
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('infrastructure_category_id') == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" 
                          class="w-full bg-[#0a0a0f] border border-white/10 rounded-lg px-4 py-2 text-white focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 transition-colors"
                          placeholder="Описание подкатегории">{{ old('description') }}</textarea>
